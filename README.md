...
graph TD
    ...
    subgraph Feature Engineering
        ...
        J & K & L & M & N & O --> P[31 features per observation]
    end
...
LightGBM dominates short-to-medium term (M+1 to M+6) with 3–4% MAPE. SARIMA is more stable at M+12 because recursive error accumulation degrades ML predictions over long horizons.

### Per Airport (Test Set 2025+, one-step)

| Airport | LightGBM Global | SARIMA | Chronos | Prophet |
|---------|----------------|--------|---------|---------|
| Lyon | 4.1% | 4.1% | 2.6% | 14.3% |
| Nantes | 6.3% | 5.3% | 4.5% | 13.0% |
| Budapest | 4.5% | 7.9% | 4.0% | 29.7% |
| Lisbon | 6.7% | 3.6% | 1.9% | 17.4% |
| Porto | 3.1% | 5.3% | 3.6% | 18.5% |
| Belgrade | 4.6% | 6.9% | 3.6% | 7.4% |

### Top Features (LightGBM Global)

1. `pax_lag_12` — same month last year (468 splits)
2. `pax_lag_1` — previous month (376)
3. `oil_price_usd` — Brent crude oil price (315)
4. `pax_yoy_growth` — year-over-year momentum (292)
5. `month_cos` — seasonal encoding (247)
...
2. **Horizon matters more than model choice.** LightGBM Recursive at M+1 (3.2% MAPE) and SARIMA at M+12 (5.2%) outperform any single model across all horizons. A production system should route by horizon.

3. **Lag-12 is the strongest predictor** — same month last year drives the forecast. Macro-economic features (oil price, unemployment) provide additional signal.
...
5. **Recursive error accumulation is real.** Porto diverges to 20.6% MAPE at M+12 due to compounding prediction errors on a high-growth airport. Log-transform or growth-rate modeling could mitigate this.

## Limitations & Next Steps

- **Porto/Budapest M+12 divergence**: recursive predictions on high-growth airports accumulate error. Potential fix: log-transform PAX before feature engineering, or add a `growth_acceleration` feature.
